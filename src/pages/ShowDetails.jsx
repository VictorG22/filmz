  ...
  return (
    <>
      ...
      >
        ...
        <div className="z-40 h-full flex max-w-[1600px] py-6 px-4">
          <div className=" flex items-end gap-2 text-[#e0e0e0]">
            ...
            <div className="mx-auto max-w-[800px] flex flex-col bg-[rgba(0,0,0,0.3)] gap-y-4 p-4 rounded-lg">
      ...
      <div className=" max-w-[1200px] mx-auto py-10 px-5 text-[#e0e0e0]">
        <div className="p-2 rounded-md shadow-2xl text-[#E0E0E0] bg-linear-to-b from-[#1a0a0f] via-[#330d14] to-[#4d0f19]">
          ...
          <div className="pt-4 grid grid-cols-[auto_1fr] gap-4">
            ...
            <span>{media.number_of_seasons ? `${media.number_of_seasons.toLocaleString()}` : "N/A"}</span>
            
            <span>Number of Episodes:</span>
            <span>{media.number_of_episodes ? `${media.number_of_episodes.toLocaleString()}` : "N/A"}</span>

            <span>Origin Country:</span>
            <span>{media.origin_country ? `${media.origin_country}` : "N/A"}</span>
            ...
            <span>{media.original_name ? `${media.original_name}` : "N/A"}</span>
            
            <span>Airing Status:</span>
            <span >{media.status ? `${media.status}` : "N/A"}</span>
            
            <span>First Air Date:</span>
            <span >{media.first_air_date ? `${new Date(media.first_air_date)?.toLocaleDateString("en-US", {month: "short", day: "numeric", year: "numeric"})}` : "N/A"}</span>
            
            <span>Last Air Date:</span>
            <span >{media.last_air_date ? `${new Date(media.last_air_date)?.toLocaleDateString("en-US", {month: "short", day: "numeric", year: "numeric"})}` : "N/A"}</span>
